...
public class OrderServiceImpl implements OrderService {
	...
	public void add(TbOrder order) {
		...
		if(cartList==null){
			throw new RuntimeException("无购物记录");
		}
		//根据购物车数据新增订单数据（有几个商家就有几个订单主表记录）
		
		double totalMoney=0;//总金额(元)
		List<String> orderIdList=new ArrayList();
		
		for(Cart cart:cartList){
			TbOrder tbOrder=new TbOrder();
			long orderId = idWorker.nextId();//得到订单ID
			...
			tbOrder.setUserId(order.getUserId());//用户账号
			
			tbOrder.setReceiverAreaName(order.getReceiverAreaName());//详细收货地址
			tbOrder.setReceiverMobile(order.getReceiverMobile());//手机号
			tbOrder.setReceiver(order.getReceiver());//收货人
			tbOrder.setSourceType("2");//PC
			...
			for(TbOrderItem orderItem:cart.getOrderItemList()){
				...
				money+=orderItem.getTotalFee().doubleValue();//累加金额
		...
		if("1".equals(order.getPaymentType())){//微信支付
			...
			payLog.setTotalFee(  (long)(totalMoney*100)  );//总金额
			payLog.setTradeState("0");//交易状态：0 未付款
